...
source("R-utility/data-generation-speckle.R")

known.variance.speckle.simulation <- function(n,
                            mean.length,
                                      seed = NA,
                                      k = 15,
                                      b = 20,
  ...
  # Simulate means
  data <- just.give.me.some.speckle.data(n = n, mean.length = mean.length, seed = seed, sd.scale = 0.05, sd.add = 0.5)
  y <- data$y
  theta <- data$theta
